...
class speedometerIntelV2Measurement(legacy_page_test.LegacyPageTest):
    def ValidateAndMeasurePage(self, page, tab, results):
        ...
        pa.start('speedometerIntelV2.ex')
        vms.start('speedometerIntelV2.vms')
...
class speedometerIntelV2(benchmark.Benchmark):
    """Speedometer benchmark V2"""
    test = speedometerIntelV2Measurement

    #speedometer 2.0
    def CreateSpeedometerV2StorySet(self, options):
        try:
            mbm = __main__.bm
        except NameError:
            mbm = bench_settings.bench_mapping()
        _URL = "http://user-awfy.sh.intel.com/awfy/ARCworkloads/Speedometer2-226694-jstc/"
        ps = story.StorySet(base_dir=os.path.dirname(__file__))
        ps.AddStory(page_mod.Page(_URL, ps, ps.base_dir, name='speedometerIntelV2'))
        return ps
    ...
    def CreateStorySet(self, options):
        ps = self.CreateSpeedometerV2StorySet(options)
        return ps
