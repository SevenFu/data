...
class speedometerIntelMeasurement(legacy_page_test.LegacyPageTest):
    def ValidateAndMeasurePage(self, page, tab, results):
        ...
        pa.start('speedometerIntel.ex')
        vms.start('speedometerIntel.vms')
...
class speedometerIntel(benchmark.Benchmark):
    """Speedometer benchmark """
    test = speedometerIntelMeasurement
    ...
    def CreateSpeedometerV2StorySet(self, options):
        ...
        _URL = "http://user-awfy.sh.intel.com/awfy/ARCworkloads/Speedometer2.0/"
        ps = story.StorySet(base_dir=os.path.dirname(__file__))
        ps.AddStory(page_mod.Page(_URL, ps, ps.base_dir, name='speedometerIntel'))
        return ps
    ...
    def CreateStorySet(self, options):
        ps = self.CreateSpeedometerStorySet(options)
        return ps
