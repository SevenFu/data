...
#class CanvasEarthIntelMeasurement(page_mod.page_test.PageTest):
class CanvasEarthIntelMeasurement(legacy_page_test.LegacyPageTest):
  def ValidateAndMeasurePage(self, page, tab, results):
    ...
    pa.start('canvasearthIntel.ex')
    vms.start('canvasearthIntel.vms')
...
class CanvasEarthIntel(benchmark.Benchmark):
  test = CanvasEarthIntelMeasurement
  ...
  def CreateStorySet(self, options):
    ...
    _URL = mbm.get_bench_URL('canvasearthIntel')
    ps = story.StorySet(base_dir=os.path.dirname(__file__))
    ps.AddStory(page_mod.Page(_URL, ps, ps.base_dir, name='canvasearthIntel'))
    return ps
