...
#class StreamingVP9IntelMeasurement(page_mod.page_test.PageTest):
class StreamingVP9IntelMeasurement(legacy_page_test.LegacyPageTest):
  def ValidateAndMeasurePage(self, page, tab, results):
    ...
    pa.start('streamingVP9Intel.ex')
    vms.start('streamingVP9Intel.vms')
...
class StreamingVP9ToughIntel(benchmark.Benchmark):
  test = StreamingVP9IntelMeasurement
  ...
  def CreateStorySet(self, options):
    ...
    ps.AddStory(page_mod.Page(_URL, ps, ps.base_dir))
    return ps


class StreamingVP9Intel(benchmark.Benchmark):
  test = StreamingVP9IntelMeasurement

  #@classmethod
  #def Name(cls):
  #  return 'streamingVP9Intel.StreamingVP9Intel'
  ...
  def CreateStorySet(self, options):
    ...
    _URL = _URL + '?id=bb1ee4'
    ps = story.StorySet(base_dir=os.path.dirname(__file__))
    ps.AddStory(page_mod.Page(_URL, ps, ps.base_dir, name="vp9-1080p-60fps"))
    return ps
