...
function gotStream(stream) {
  ...
  localFpsCal = setInterval(function() {
    if (localVideo.readyState > HTMLMediaElement.HAVE_CURRENT_DATA && !localVideo.paused) {
      localVideoFps = localVideo.webkitDecodedFrameCount - decodedLocalFrames;
      decodedLocalFrames = localVideo.webkitDecodedFrameCount; 
      console.log('The FPS of local video captured: ' + localVideoFps);
    }
  }, 1000);
}
...
setInterval(function() {
  if (remotePeerConnection && remotePeerConnection.getRemoteStreams()[0]) {
    remotePeerConnection.getStats(function(results) {
      var getStatsResult = results.result();
      var statsString = dumpStats(getStatsResult);
      ...
      if (activeCandidatePair && activeCandidatePair.remoteCandidateId) {
        remoteCandidate = getStatsResult[activeCandidatePair.remoteCandidateId];
      }
